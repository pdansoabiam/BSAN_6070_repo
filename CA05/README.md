...
A k-Nearest Neighbors recommender system built with Python and scikit-learn that finds the 5 most similar movies to a given query movie based on genre and IMDB rating data.
...
| Records 	| 30 movies 										|
| URL 		| `https://github.com/ArinB/MSBA-CAData/raw/main/CA05/movies_recommendation_data.csv` 	|
...
└── movies_recommendation_data.csv  												Auto-loaded from GitHub URL
...
The query movie may exist in the dataset. Setting `n_neighbors=6` and skipping `indices[0][0]` (the self-match at distance 0) ensures we always return exactly 5 meaningful recommendations.
...
To query a different movie, update Cell 6 with that movie's genre values and IMDB rating, then re-run Cells 7–9.
